        ...
        <div className="row gy-4 " >
          ...
          <div className="col-lg-12 text-capitalize col-md-12" key={index}>
            <div className="card p-4  position-relative">
              ...
              <hr/>
              <div className="d-flex justify-content-between">
              <h5>Status is {el?.data?.status==1?"Pending Yet!!":el?.data?.status==2?"Approved!! They will contact you soon.":el?.data?.status==3?"Completed.":"Oops!! Due to some reason Your booking is rejected. "}</h5>
              <div>
                {el?.data?.status==3 &&
                <Link className="btn btn-primary" to={el?.data?.finalPost}>View Post</Link>
                }
              </div>
              </div>
